...
export class ItemTypeService {
  ...
  async createItemType(body: CreateItemTypeDTO) {
    try {
      ...
      this.logRepository.insertOne(logError({ method: 'ITEM TYPE SERVICE-createItemType', error }))
  ...
  async updateItemType(body: UpdateItemTypeDTO) {
    try {
      ...
      this.logRepository.insertOne(logError({ method: 'ITEM TYPE SERVICE-updateItemType', error }))
      throw new InternalServerErrorException(error.message)
    }
  }
  ...
  async getListItemType() {
    try {
      ...
      this.logRepository.insertOne(logError({ method: 'ITEM TYPE SERVICE-getListItemType', error }))
      throw new InternalServerErrorException(error.message)
    }
  }
}
